...
>(({ className, children, position = "popper", enableSearch = false, ...props }, ref) => {
    ...
    return React.Children.map(children, child => {
      ...
      if (child.type === SelectGroup) {
        const filteredGroupChildren = React.Children.toArray(child.props.children).filter(groupChild => {
          ...
          const childText = String(groupChild.props.children || "").toLowerCase()
  ...
  }, [children, enableSearch, searchQuery])
  ...
  return (
    <SelectPrimitive.Portal>
      ...
      >
        {enableSearch && (
          <div className="flex items-center px-2 py-2 sticky top-0 bg-popover border-b">
            <Search className="h-4 w-4 opacity-50 mr-2" />
            <Input
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              placeholder="Search..."
              className="h-8 px-2 text-xs border-none focus-visible:ring-0 focus-visible:ring-offset-0"
            />
          </div>
        )}
