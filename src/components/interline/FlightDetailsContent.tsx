        ...
        <div key={segment.id} className="mb-8 last:mb-0">
          <div className="flex items-center gap-2 mb-3">
            ...
            <div className="flex flex-col md:flex-row md:items-center md:gap-2">
              <p className="font-medium text-base text-chamDarkBlue">{segment.airline} ({segment.airlineCode})</p>
          ...
          <div className="grid grid-cols-7 gap-4 ml-8 bg-white p-4 rounded-lg shadow-sm border border-blue-100/50">
            <div className="col-span-3">
              <p className="text-xl font-medium text-chamDarkBlue">{format(segment.departureTime, "HH:mm")}</p>
              <p className="text-sm font-medium text-gray-600">{format(segment.departureTime, "dd MMM yyyy")}</p>
              <p className="text-sm text-chamBlue font-medium mt-1">{segment.fromCode}</p>
            ...
            <div className="col-span-3 text-right">
              <p className="text-xl font-medium text-chamDarkBlue">{format(segment.arrivalTime, "HH:mm")}</p>
              <p className="text-sm font-medium text-gray-600">{format(segment.arrivalTime, "dd MMM yyyy")}</p>
              <p className="text-sm text-chamBlue font-medium mt-1">{segment.toCode}</p>
          ...
          {index < flight.segments.length - 1 && (
            <div className="flex items-center justify-center py-3 px-4 bg-gradient-to-r from-orange-50 to-amber-50 rounded-md mt-4 mb-6 ml-8 border-l-4 border-orange-400 shadow-sm">
              <div className="flex items-center gap-2 text-sm text-orange-700">
                ...
                <span className="font-medium">Connection Time: {flight.connectionTime} in {segment.to} ({segment.toCode})</span>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="flex flex-col gap-3 bg-gradient-to-r from-chamBlue/5 to-blue-50 p-5 rounded-lg shadow-sm border border-blue-100">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="px-2 py-0.5 bg-chamBlue/10 rounded text-sm text-chamBlue">
              {flight.cabin === 'economy' ? 'Economy Class' : 'Business Class'}
            </span>
          </div>
        </div>
      </div>
